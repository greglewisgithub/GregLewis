...
# Deployment script for Finance Ops SQL layers.
# Usage:
#   ./scripts/deploy_financeops.sh \
#     --db-host "$DEV_DB_HOST" \
#     --db-user "$DEV_DB_USER" \
...
LOG_FILE="$ROOT_DIR/scripts/deploy_financeops_$(date +%Y%m%d_%H%M%S).log"
...
echo "Starting Finance Ops deployment at $(date -u +"%Y-%m-%dT%H:%M:%SZ")" | tee -a "$LOG_FILE"
...
echo "Finance Ops deployment completed successfully at $(date -u +"%Y-%m-%dT%H:%M:%SZ")" | tee -a "$LOG_FILE"
